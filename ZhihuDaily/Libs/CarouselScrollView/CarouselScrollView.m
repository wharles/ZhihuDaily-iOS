...
NSString* const ID = @"CarouselCell";

@interface CarouselScrollView ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
+ (instancetype)carouselScrollViewWithFrame:(CGRect)frame imagesGroup:(NSArray *)imagesGroup {
    CarouselScrollView* carouselScrollView = [[self alloc] initWithFrame:frame];
    carouselScrollView.imagesGroup = [NSMutableArray arrayWithArray:imagesGroup];
    return carouselScrollView;
}

+ (instancetype)carouselScrollViewWithFrame:(CGRect)frame imageURLStringsGroup:(NSArray *)imageURLStringsGroup {
    CarouselScrollView* carouselScrollView = [[self alloc] initWithFrame:frame];
    carouselScrollView.imageURLStringsGroup = [NSMutableArray arrayWithArray:imageURLStringsGroup];
    return carouselScrollView;
}
...
- (void)initialization
{
    _pageControlAliment = CarouselScrollViewPageControlAlimentCenter;
    _autoScrollTimeInterval = 2.0;
    _titleLabelTextColor = [UIColor whiteColor];
    _titleLabelTextFont= [UIFont fontWithName:@"AvenirNextCondensed-Regular" size:18];
    ...
    _pageControlStyle = CarouselScrollViewPageControlStyleAnimated;
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[CarouselCollectionViewCell class] forCellWithReuseIdentifier:ID];
...
- (void)setPageControlStyle:(CarouselScrollViewPageControlStyle)pageControlStyle
{
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case CarouselScrollViewPageControlStyleAnimated:
        {
        ...
        case CarouselScrollViewPageControlStyleClassic:
        {
...
- (void)layoutSubviews
{
    ...
    CGFloat x = (self.actualWidth - size.width) * 0.5;
    if (self.pageControlAliment == CarouselScrollViewPageControlAlimentRight) {
        x = self.mainView.actualWidth - size.width - 10;
    }
    CGFloat y = self.mainView.actualHeight - size.height - 10;
...
{
    CarouselCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.delegate respondsToSelector:@selector(carouselScrollView:didSelectItemAtIndex:)]) {
        [self.delegate carouselScrollView:self didSelectItemAtIndex:indexPath.item % self.imagesGroup.count];
    }
}
...
{
    int itemIndex = (scrollView.contentOffset.x + self.mainView.actualWidth * 0.5) / self.mainView.actualWidth;
